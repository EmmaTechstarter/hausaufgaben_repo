...
app.put("/tiere/:id", (req, res) => {
    ...
    db.run(
        ...
        function (err) {
            if (err) {
                res.status(500).send("Fehler beim Aktualisieren des Tieres");
            } else if (this.changes === 0) {
                res.status(404).send("Tier nicht gefunden");
            } else {
                res.send("Tier wurde erfolgreich aktualisiert");
            }
        }
    );
});
...
app.delete("/tiere/:id", (req, res) => {
    ...
    db.run("DELETE FROM tiere WHERE id = ?", [id], function (err) {
        if (err) {
            res.status(500).send("Fehler beim Löschen des Tieres");
        } else if (this.changes === 0) {
            res.status(404).send("Tier nicht gefunden");
        } else {
